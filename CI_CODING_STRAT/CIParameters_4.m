classdef CIParameters_4
    ...
    properties
        ...
        n=[12]; % activated electrodes in one cycle. CAVE: If Channels are deactivated, n has to be adjusted: 1 electrode deactivated means n = m-1
        pps=[3000*ones(1,12)]; % stimulation rate. Currently only one rate can be chosen for all electrodes
        Distance; % Wenn eine Rate genutzt wird
        ipg=2.1*10^-6;; % inter phase gap
        PulseWidth=7; % In this case fake PW for preprocessing of electrodogram
        PulseWidthWrapper=40; % "real PW"
        CIFs=16000;
        ...
        vol=0.8; % volume control limits C-level: 1 = full dynamic range is used
    end
end
